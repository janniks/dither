...
| _pending_ | Phase 7: loop detection (`LoopDetector` with TTL + threshold) wired into daemon fire path; halts surfaced in status snapshot's `recentHalts`; richer `dither daemon status` view with schedule/watch entries + recent halts + recent failures.                                                                |
