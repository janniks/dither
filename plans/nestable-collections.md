...
| 3856abe | Phase 2 — manifest = default seed, not ceiling. Drops subset-of-manifest enforcement from `resolveAllowList` for `net` and `collections`. New widening test pins the behavior. Docs sweep across `plugins/{index,authoring}.mdx`, `cli/plugin.mdx`, `concepts/{security,collections}.mdx`. 41/41 tests, lint, typecheck green.                     |
