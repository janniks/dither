...
**Acceptance:**
- [ ] `extract.test.ts` covers four canned-HTML cases without network.
- [ ] `render.test.ts` covers two cases — single-parent and two-parent
      dedupe — and asserts the parent fields are always lists.
- [ ] `deno test` runs from the plugin directory and is green.
- [ ] No mocks of `fetch`; tests are pure-input/output.
