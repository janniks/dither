...
| `a668ac6` | Phase 4 — strip qmd's `@@ -X,Y @@` diff header by plucking just `lines[s.line - 1]` from body; fallback to first non-empty line when match is empty or qmd throws. Unit tests for `safeSnippet` (empty body, no-match fallback, match path) + integration test for non-matching-chunk hit. 18/18 search tests pass. |
