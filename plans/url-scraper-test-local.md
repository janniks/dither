...
| (pending) | Post-plan amendment: `SKIP_DOMAINS` env (default `x.com,twitter.com`) added on Phase-4 anti-bot finding. Cache extended with `skipped: true`; pre-fetch + post-fetch host check. Verified: 5/12 URLs now skip-domain (vs scrape useless block pages); second run is true no-op including domain skips. |
