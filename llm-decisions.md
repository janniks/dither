...
| `<library>/<collection>/...`                | canonical markdown store (library)             | Path configurable per-install. Default `<dither-home>/library/`; `dither init --library <path>` adopts an external directory (canonicalised via `realpath`). qmd indexes the library tree.                                                                                                                                                                                                                       |
| `<dither-home>/config.json`                 | schema + library path                          | Two-level JSON; `library.path` is the only key in v1. Written by `dither init`. Required first step.
| `<dither-home>/qmd-index.sqlite`            | qmd index over the library                     | Always in dither home, never in the library — even when the library is external. Index reuse with qmd-CLI is parked (see `notes/qmd-index-reuse.md`).
| `<dither-home>/env.json`                    | global env store                               | Plugins read via `--allow-env` grants.
| `<library>/attachments/<uuid>/<filename>`   | raw attachment blobs (parked)                  | Content-addressed by UUID; original filename preserved for human inspection.                                                                                                                                                                                                                              |
| `<dither-home>/plugins/<name>/`                 | installed plugin code (immutable post-install) | Author-owned tree.                                                                                                                                                                                                                                                                                        |
| `~/.dither/plugins/<name>/state/state.json` | plugin's persistent state                      | Granted to plugin via Deno flags.                                                                                                                                                                                                                                                                         |
| `<dither-home>/grants/<name>.json`          | dither-owned grants per plugin                 | Outside the plugin dir so plugins can't tamper with their own grants.                                                                                                                                                                                                                                     |
| `<dither-home>/runs/<run-id>/`              | ephemeral plugin run scratch                   | Cleaned after promote.
| `<dither-home>/locks/`                      | per-plugin lockfiles                           | Serialise concurrent runs of the same plugin.
| `<dither-home>/bin/deno-<version>`          | managed Deno binary                            | dither downloads and SHA-verifies its own pinned Deno on first plugin install/run; spawn target for plugin processes. `DITHER_USE_SYSTEM_DENO=1` opts out.
| `<dither-home>/logs/`                       | daemon and detached-run logs                   |
| `<dither-home>/status.json`                 | daemon status snapshot                         |                                                                                                                                                                                                                                                                                    |
| `<dither-home>/keys/dither.sqlite`          | API key store (parked)                         | SQLite for atomic ops; only metadata + hashed tokens.                                                                                                                                                                                                                                                     |
| `<dither-home>/collections.json` (parked)   | collection id↔name registry                    | Enables both name-based and id-based grants. Not yet written.                                                                                                                                                                                                                                                              |
| `<dither-home>/dither.sqlite` (parked)      | daemon state (run history, schedules)          | Separate DB from keys to keep blast radius low. Today: per-run journal under `runs/<run-id>/` plus aggregated `status.json`; SQLite store deferred.                                                                                                                                                                                                                                                           |
| `<dither-home>/dither.sock` (parked)        | unix socket (CLI ↔ daemon IPC)                 |                                                                                                                                                                                                                                                                                                           |
| `<dither-home>/dither.pid`                  | daemon PID file                                | For self-respawn pattern.                                                                                                                                                                                                                                                                                 |

qmd's own (separate) CLI index at `~/.cache/qmd/index.sqlite` is never touched by dither. A user who runs qmd-CLI over the same library ends up with two independent indices over the same files; sharing a single index file is parked — see `notes/qmd-index-reuse.md`.
...
- **Plugin name auto-stamped to `source` frontmatter** by the SDK using `DITHER_PLUGIN_NAME` env. Plugins cannot fake another plugin's source — the host validates `source === <plugin name>` at promote, before the file moves into the library.
