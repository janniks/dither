...
const VARIANTS = [
  ...
  },
  {
    label: "No lock-in",
    intro:
      "Your archive stays yours: plain markdown on disk, no database, no lock-in. Delete dither tomorrow and everything still reads fine.",
  },
  {
    label: "Search",
    intro:
      "Plugins write markdown into your library and dither indexes it in place — search and sort everything without a second copy.",
  },
  {
    label: "Obsidian",
    intro:
      "Point it at your Obsidian vault and it's indexed in place, searchable alongside plugin-written markdown. No database, no copies.",
  },
  {
    label: "Minimal",
    intro:
      "Markdown in, markdown out — plugin entries and your Obsidian vault, indexed in place. No database, no proprietary format.",
  },
];
...
export default function JustMarkdownIntroPrototypePage() {
    ...
    <div className="min-h-screen bg-fd-background text-fd-foreground">
      ...
      <div className="mx-auto flex w-full max-w-[1080px] flex-col gap-24 px-6 py-16 pb-40">
        ...
        <div key={`${index}-${replayCount}`}>
          <Section intro={VARIANTS[index].intro} />
        </div>
