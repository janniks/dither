...
The library path is recorded in `<config-dir>/config.json` and chosen at `dither init`. The config dir resolves as `$DITHER_DIR` → `$XDG_CONFIG_HOME/dither` → `~/.dither`. By default the library is `$DITHER_DIR/library/` (or `~/.dither/library/`); with `dither init --library <path>` it's wherever you point it. See [Storage layout](/docs/concepts/storage) for the config-dir / library split.
...
That's the install-time **default**. At install you can override or extend it via `--create` (and `--edit` for overwriting entries other plugins created):

```bash
dither plugin install ./my-plugin --create 'messages/tom/**'
```
...
dither doesn't store entries in a database. Search is provided by [qmd](https://github.com/tobi/qmd), which builds a SQLite index over the directory tree. Every top-level subdirectory of the library AND every registered external collection is registered as a qmd collection on open, and qmd indexes `**/*.md` underneath each.

The index file lives at `<config-dir>/qmd-index.sqlite` — in the config dir, not the library. Don't edit it; it's regenerable from your markdown. See [Storage](/docs/concepts/storage).
...
If you have an installed plugin with a `watch` block (from its manifest, or added via `dither plugin run --watch`) that points at the collection you're editing, the daemon will pick up your changes via its file watcher and fire that plugin — useful when intentional, surprising when not. Plugins with no `watch` block (the common case for ingest plugins) don't react to hand edits.
