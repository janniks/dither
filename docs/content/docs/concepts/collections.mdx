...
If you have an installed plugin with a `watch` block (from its manifest, or added via `dither plugin run --watch`) that points at the collection you're editing, the daemon will pick up your changes via chokidar and fire that plugin — useful when intentional, surprising when not. Plugins with no `watch` block (the common case for ingest plugins) don't react to hand edits.
