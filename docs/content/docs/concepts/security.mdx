...
2. **Grants** — written by dither at install time to `<config-dir>/grants/<name>.json`. This is the **actual** allowance for this plugin on this machine, and the only thing that gates promote at run time. When you pass a grant flag at install, that's the grant; when you don't, the manifest's declaration is used as the default. The user is the source of truth — the manifest cannot constrain a grant the user explicitly authorized.
3. **Globals** — managed env values shared across plugins, stored in `<config-dir>/env.json`. A plugin granted `--allow-env OPENAI_API_KEY` reads the current global value at every run. Rotate the global once; every plugin sees the new value next run. See [`dither env`](/docs/cli/env).

The config dir is resolved once, first match wins: `$DITHER_DIR`, then `$XDG_CONFIG_HOME/dither`, then `~/.dither`.
...
A list of names in `<config-dir>/env.json` the plugin is allowed to read. Granted via `--allow-env KEY,KEY2`. At every run, dither resolves each name to its current global value and adds it to `input.env`. If the global isn't set, the plugin sees nothing for that key.
...
### `create` — promote allowlist

A list of collection globs supplied via `--create NAME,NAME2`. Enforced **at promote time, not at write time**: a plugin can write any `*.md` it likes to its run dir; dither inspects each file's `collection:` frontmatter after the run and rejects the whole batch if any file targets a collection no pattern covers. Patterns are globs over the collection-path namespace (`notes`, `feeds/*`, `messages/**`); a `<X>/**` pattern also covers the bare `<X>`.

### `edit` — overwrite allowlist

A list of collection globs supplied via `--edit NAME,NAME2`. A `create` grant only lets a plugin write entries it owns. Overwriting an existing entry whose `source:` is a **different** plugin additionally requires an `edit` grant covering that collection; without one, dither skips that output rather than clobbering it. Grants beyond the manifest are flags-only — the interactive install flow never offers them.
...
- Read its own installed source dir, its run scratch dir, any path in `grants.files`, and its watch roots.
- Write to its run scratch dir — and nowhere else. Its persistent state is written to a run-local copy there; dither promotes it back after the run.
- Read the always-granted `DITHER_*` env vars (only — see below).
- Reach the network only on hosts in `grants.net`.

A plugin **cannot**:

- Read or write anywhere else under the config dir or the library — including other collections, `grants/`, `env.json`, other plugins, or the qmd index.
...
Grants only change through an install. A plugin whose manifest asks for more than last time has to be reinstalled, and the interactive review shows each newly-requested entry as `(new)` and unchecked — you re-approve it explicitly or it isn't granted.
...
3. `collection:` is set, passes path validation (no `..`, no leading/trailing `/`, allowed charset, no `.md` suffix), and matches at least one glob pattern in `grants.create`.
4. If the destination file already exists and was written by a **different** `source:`, at least one pattern in `grants.edit` covers the collection.

Failures on 1–3 throw and stop the promote. A failure on 4 skips that one output (journaled as a skip) and leaves the existing entry untouched. The run dir is removed regardless.
