...
3. **Globals** — managed env values shared across plugins, stored in `<dither-home>/env.json`. A plugin granted `--allow-env OPENAI_API_KEY` reads the current global value at every run. Rotate the global once; every plugin sees the new value next run. See [`dither env`](/docs/cli/env).
