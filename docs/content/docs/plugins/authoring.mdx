...
`--detach` forks the run into the background and returns immediately. Stdout and stderr are captured to `<dither-home>/logs/<plugin>-<timestamp>.log`. The plugin keeps running after your shell exits; tail the log to watch progress.
