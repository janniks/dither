...
| Field      | Type                                       | Notes                                                                                                                          |
| ---------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `schedule` | `string` (cron expression or `daily`/`hourly`) | Fired by the dither daemon. Installing a plugin with `schedule` lazily starts the daemon. Manual `dither plugin run` still works. |
...
- **Detached runs aren't supervised after the fork.** `--detach` writes a log file and `unref()`s the child, so there's no `dither plugin stop`. The run is still recorded in `<dither-home>/history/<runId>/` — use `dither runs list` / `dither runs tail` to inspect it, or tail the printed log path; kill via `kill <pid>` if needed.
