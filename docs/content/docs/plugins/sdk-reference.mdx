...
```ts
function readInput<C = Record<string, unknown>, S = Record<string, string>>(): Promise<
  PluginInput<C, S>
>;
```
...
Reads a file the user supplied as a `files[]` input. The SDK looks up the absolute path in `input.json.files[inputId]` (which the host already added to Deno's `--allow-read` allowlist) and returns the UTF-8 contents.

Saves the two-step pattern of importing `node:fs/promises` and indexing into `input.files` by hand.

**Behavior:**

- Throws if the input id is not present in `input.json.files` (i.e. not declared in the manifest's `files[]`, or not provided at install time, or not required and not supplied).
