...
- **Scheduled** — `schedule: "*/15 * * * *"` (or `"daily"` / `"hourly"`) in the manifest. The dither daemon fires the plugin on that cron. If you install a plugin with a `schedule`, the CLI lazily starts the daemon so it picks up the new entry.
- **Watched** — `watch: { collections: [...], glob: "..." }`. The daemon uses chokidar to watch the declared collections under the library and fires the plugin when matching files change. The triggering paths land in `input.targets`.
