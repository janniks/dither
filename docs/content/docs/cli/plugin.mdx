...
Plugins are small Deno programs that write markdown into collections you've granted them. The CLI manages four lifecycle actions. All four refuse with the standard pre-init error until [`dither init`](/docs/cli/init) has run.
...
| `--detach`           | Fork the run into the background and return immediately. Stdout/stderr go to `<dither-home>/logs/<name>-<ts>.log`. |
